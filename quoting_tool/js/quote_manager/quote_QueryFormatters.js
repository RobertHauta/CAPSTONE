...
function updateLaborRequest(LabId){
    var totLabor = parseFloat($("#TotLabor").text().substring(1).replace(/,/g, ""));
    var margin = parseFloat($("#Margin").text().slice(0, -1));
...
function createLaborRequest(QID){
    ...
    // Fields to populate in the new record
    record.priceperunit = Number(parseFloat(totLabor.toFixed(2))); // Currency
    record.msdyn_estimatedcost = Number(parseFloat(totLabor.toFixed(4))); // Currency
    record.productname = "Labour Costs"; // Text
    record.msdyn_costtotal = Number(parseFloat(LaborPlus.toFixed(2))); // Currency
    record.quantity = 1; // Decimal
    record.producttypecode = 5; // Choice
    record.extendedamount = Number(parseFloat(totLabor.toFixed(4)));
    record.msdyn_budgetamount = Number(parseFloat(totLabor.toFixed(2))); // Currency
