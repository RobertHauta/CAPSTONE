 ...
 *
 * Description: This file contains all functions for creating the PDF template
 *
...
function handleSummary(){
    $('#pdf-summary').css("display", "none");
    const parent = $('#pdf-summary').parent();
    let first = true;
    let maxLength = 8000;
    let currentLength = 0;
    let newParagraph = $('<p>', {class: 'pdf-section summary-saving'});
    
    function appendParagraph(paragraph){
        if(first){
            paragraph.removeClass("pdf-section");
    ...
    // Iterate over child nodes of the original <p>
    function processNode(node){
        if(node.prop('nodeName') === 'DIV'){
            newParagraph.append("<br>");
            node.contents().each(function () {
                processNode($(this)); // Recursion happens here
            });
        } else if (node[0].nodeType === Node.TEXT_NODE) {
            // Split text node based on length
            const words = node.text().split(/\s+/);
            words.forEach((word) => {
                if (currentLength + word.length + 1 <= maxLength) {
                    newParagraph.append(document.createTextNode(word + ' '));
                    currentLength += word.length + 1;
                } else {
                    // Append new <p> and start another
                    appendParagraph(newParagraph);
                    newParagraph = $('<p>', {class: 'pdf-section summary-saving'});
                    newParagraph.append(document.createTextNode(word + ' '));
                    currentLength = word.length + 1;
                }
            });
        } else if (node.prop('nodeName') === 'UL' || node.prop('nodeName') === 'OL'){
            // Clone and append list nodes
            if (currentLength + node.text().length <= maxLength) {
                newParagraph.append(node.clone());
                currentLength += node.text().length;
            } else { //Split list if too long
                let firstHalf = $(`<${node.prop('nodeName').toLowerCase()}>`);
                let secondHalf = $(`<${node.prop('nodeName').toLowerCase()}>`);

                node.children().each(function () {
                    const child = $(this);
                    if(currentLength + child.text().length <= maxLength) {
                        firstHalf.append(child.clone());
                    }
                    else{
                        secondHalf.append(child.clone());
                    }
                    currentLength += child.text().length;
                });
                newParagraph.append(firstHalf);
                
                appendParagraph(newParagraph);
                newParagraph = $('<p>', {class: 'pdf-section summary-saving'});
                newParagraph.append(secondHalf);
                currentLength = secondHalf.text().length;
            }
        } else {
            // Clone and append non-text nodes (preserves tags)
            if (currentLength + node.text().length <= maxLength) {
                newParagraph.append(node.clone());
                currentLength += node.text().length;
            } else {
    ...
    $('#pdf-summary').contents().each(function () {
        console.log($(this));
        processNode($(this)); // $(this) refers to the current node
    });    
...
function revertSummary(){
    $(".summary-saving").remove();
    $('#pdf-summary').css("display", "block");
}
